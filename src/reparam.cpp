...
Reparam::setHyperparameter(std::string key, std::string value)
{
  ...
  if (key == "lambda_reg_phi_h") {
    lambda_reg_phi_h = std::stod(value);
  } else if (key == "lambda_reg_phi_J") {
    lambda_reg_phi_J = std::stod(value);
...
Reparam::writeHyperparameters(std::string output_file, bool append)
{
  ...
  // Header
  stream << std::endl;
  stream << "[[reparametrization]]" << std::endl;

  // BM settings
  stream << "lambda_reg_phi_h=" << lambda_reg_phi_h << std::endl;
  stream << "lambda_reg_phi_J=" << lambda_reg_phi_J << std::endl;
...
Reparam::compareHyperparameter(std::string key, std::string value)
{
  ...
  // arrays, not actual types.
  if (key == "lambda_reg_phi_h") {
    same = same & (lambda_reg_phi_h == std::stod(value));
  } else if (key == "lambda_reg_phi_J") {
    same = same & (lambda_reg_phi_J == std::stod(value));
  } else if (key == "initial_params") {
...
Reparam::updateGradients(void)
{
  ...
  for (int i = 0; i < N; i++) {
    for (int aa = 0; aa < Q; aa++) {
      double phi = 2 * params.h(aa, i);
      for (int j = 0; j < N; j++) {
        if (i < j) {
          for (int bb = 0; bb < Q; bb++) {
            phi += params.J(i, j)(aa, bb) * training->frequency_1p(bb, j);
          }
        } else if (i > j) {
          for (int bb = 0; bb < Q; bb++) {
            phi += params.J(j, i)(bb, aa) * training->frequency_1p(bb, j);
          }
        }
      }
      double delta = samples->frequency_1p(aa, i) -
                     training->frequency_1p(aa, i) +
                     lambda_reg_phi_h * 0.5 * phi;
        ...
        for (int aa2 = 0; aa2 < Q; aa2++) {
          double delta =
            -(training->frequency_2p(i, j)(aa1, aa2) -
              ...
              (samples->frequency_1p(aa1, i) - training->frequency_1p(aa1, i)) *
                training->frequency_1p(aa2, j) +
              (samples->frequency_1p(aa2, j) - training->frequency_1p(aa2, j)) *
                training->frequency_1p(aa1, i) -
              lambda_reg_phi_J * params.J(i, j)(aa1, aa2));
    ...
    for (int i = 0; i < N; i++) {
      for (int aa = 0; aa < Q; aa++) {
        ...
        for (int j = 0; j < N; j++) {
          if (i < j) {
            for (int bb = 0; bb < Q; bb++) {
              phi += params.J(i, j)(aa, bb) * validation->frequency_1p(bb, j);
            }
          } else if (i > j) {
            for (int bb = 0; bb < Q; bb++) {
              phi += params.J(j, i)(bb, aa) * validation->frequency_1p(bb, j);
            }
          }
        }
        double delta = samples->frequency_1p(aa, i) -
                       validation->frequency_1p(aa, i) +
                       lambda_reg_phi_h * 0.5 * phi;
        ...
        for (int aa1 = 0; aa1 < Q; aa1++) {
          for (int aa2 = 0; aa2 < Q; aa2++) {
            double delta = -(validation->frequency_2p(i, j)(aa1, aa2) -
                             samples->frequency_2p(i, j)(aa1, aa2) +
                             (samples->frequency_1p(aa1, i) -
                              validation->frequency_1p(aa1, i)) *
                               validation->frequency_1p(aa2, j) +
                             (samples->frequency_1p(aa2, j) -
                              validation->frequency_1p(aa2, j)) *
                               validation->frequency_1p(aa1, i) -
                             lambda_reg_phi_J * params.J(i, j)(aa1, aa2));
...
Reparam::updateParameters(void)
{
  ...
  for (int i = 0; i < N; i++) {
    for (int a = 0; a < Q; a++) {
      params.h(a, i) +=
        learning_rates.h(a, i) * gradient.h(a, i) + 0.5 * Dh(a, i);
    }
  }
};
