...
SGDM::compareHyperparameter(std::string key, std::string value)
{
  ...
  if (key == "lambda_reg_h") {
    ...
    same = same & (beta_J == std::stod(value));
  } else if (key == "learn_rate_h") {
...
SGDM::updateMoments(void)
{
  for (int i = 0; i < N; i++) {
    for (int j = i + 1; j < N; j++) {
      for (int a = 0; a < Q; a++) {
        for (int b = 0; b < Q; b++) {
          moment1.J(i, j)(a, b) =
            gradient.J(i, j)(a, b) + beta_J * moment1.J(i, j)(a, b);
        }
      }
    }
  }
  ...
  for (int i = 0; i < N; i++) {
    for (int a = 0; a < Q; a++) {
      moment1.h(a, i) = gradient.h(a, i) + beta_h * moment1.h(a, i);
    }
  }
};
