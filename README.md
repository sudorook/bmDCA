...
- `-t`: sequence similarity threshold for re-weighting (if no weights file is
  provided)
...
- `-s`: maximum sequence identity fraction, where sequences above the threshold
  will be pruned from the alignment
...
**Important:** `process_msa` does not handle gaps represented by '.' characters.
...
**Note:** When pruning position and sequence gaps, gapped positions are removed
...
positions (fields, h) and pairs of frequencies at pairs of positions (couplings,
J).
...
**Important:** Due to the sheer number of parameters being trained versus the
number of sequences as input, there is a very substantial. To help keep track of
overfitting at each step of the inference loop, bmDCA will by default split out
100 (by default) effective sequences chosen at random to use as a validation
set. Overfitting becomes more severe as the difference in sequence energy
between the training and validation set increases.

#### Example 1: Run from FASTA file

To learn a FASTA-formatted multiple sequence alignment (with re-weighting) and a
config file:
...
**Note:** To guarantee that inferences loop produce the same results
...
1. `save_period` - the interval at which the state of the program is saved to
   disk.
2. `save_best_step` - boolean flag to save any step that produces the lowest RMS
   error with respect to the MSA so far in the inference.
...
- `-p`: input parameters, text format **or** fields (h) parameters file, binary
  format
...
3. `alpha_reg` - relative weighting of L1 (`alpha_reg=0`) and L2 (`alpha_reg=1`)
   regularization (default: 1)
...
enhance movement in directions that do little or enhance model accuracy. Fast,
but tends to overfit small alignments very quickly.
...
down by the moments and are less able to affect inference. Weight decay (after a
transform of the original L2 loss minimization equation) is independent of the
moments.
...
7. `update_burn_times` - flag to check tune the burn times during runtime using
   MCMC sample energies and autocorrelations (walkers > 1) or lookahead sampling
   of a small set of sequences (walkers == 1). (default: true)
8. `adapt_up_time` - multiple to increase MCMC wait/burn-in time (default: 1.5)
9. `adapt_down_time` - multiple to decrease MCMC wait/burn-in time (default 0.6)
...
- `bmdca_run.log`: table of values and measurements taken at every iteration of
...
`parameters_final.txt`. **Use this file or the latest learned parameters for
...
9. `auto-cross-err` - combined deviation for correlations within and between
   trajectories
10. `energy-start-avg` - mean energy after `burn-in` steps (start of trajectory)
...
22. `validate-err-tot-min` - smallest total RMSE found so far for the validation
    MSA
...
See the above usage section for how to use `arma2ascii`. Parameters converted by
the program will match the format for parameters generated when
`output_binary=false` is specified in the config file. See the below section for
details.
...
indices go from 0 to 20 (21 amino acids total, including gaps). 0 corresponds to
a gap.

### Sequence statistics

The sequence statistics files (e.g. `stat_align_1p.txt` and `stat_align_2p.txt`)
...
where `[amino acid frequencies (21)]` is a row of frequencies for each of the 21
amino acids.
...
**For users of shared resources:** OpenMP will default to the number of
...
manager**, such as Slurm or Sun Grid Engine. The manager will limit bmDCA to the
number of cores in the job request, so manipulating `OMP_NUM_THREADS` is not
needed.
